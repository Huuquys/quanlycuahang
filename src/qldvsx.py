...
class sanxuat_w(QMainWindow):
    sx = QtCore.pyqtSignal()
    def __init__(self):
        super(sanxuat_w, self).__init__()
        uic.loadUi('view/qldvsx.ui', self)
        
        # tableWidget
        self.tableWidget = self.findChild(QtWidgets.QTableWidget, "tb_qldvsx")
        ...
        self.btn_thoat_qldvsx.clicked.connect(self.Exit)
        self.btn_them_qldvsx.clicked.connect(self.them)
        self.btn_sua_qldvsx.clicked.connect(self.sua)
        self.btn_xoa_qldvsx.clicked.connect(self.xoa)
        self.btn_timkiem_qldvsx.clicked.connect(self.timkiem)
    
    # show data lên table
    def loaddata(self):
        sql = "SELECT * FROM qldvsx"
        cur.execute(sql)
    ...
    def display_row(self):
        ...
        if current_row >= 0:
            ...
            self.txt_qldvsx_ten.setText(row_data[1])
            self.txt_qldvsx_dc.setText(row_data[2])
            self.txt_qldvsx_sdt.setText(row_data[3])
    
    def them(self):
        ten = self.txt_qldvsx_ten.text ()
        dc = self.txt_qldvsx_dc.text()
        sdt = self.txt_qldvsx_sdt.text()
        
        sql = "INSERT INTO qldvsx (ten, diachi, sdt) VALUES (%s, %s, %s)"
    ...
    def sua(self):
        ...
        if current_row >= 0:
            ...
            sql = "UPDATE qldvsx SET ten = %s, diachi = %s, sdt = %s WHERE id = %s"
    ...
    def xoa(self):
        ...
        if current_row >= 0:
            ...
            sql = "DELETE FROM qldvsx WHERE id = %s"
            cur.execute(sql, (id,))
            db.commit()
            self.loaddata()
    
    def timkiem(self):
        search_criteria = self.tim_qldvsx.text().split()
        sql = "SELECT * FROM qldvsx WHERE 1=1"
